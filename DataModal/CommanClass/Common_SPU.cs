       ...
       public static string EntrySource = "Web";
        ...
        public static PostResponse fnSetEMP_TalentPool_Attachments(FileResponse Modal)
        {
            ...
            using (SqlConnection con = new SqlConnection(ClsCommon.ConnectionString()))
            {
                ...
                {
                    con.Open();
                    using (SqlCommand command = new SqlCommand("spu_SetEMP_TalentPool_Attachments", con))
                    {
                        SqlDataAdapter da = new SqlDataAdapter();
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.Add("@ID", SqlDbType.Int).Value = Modal.ID ?? 0;
        ...
        public static DashboardItems.ForAll GetDashboard_Common(GetResponse Modal)
        {
            DashboardItems.ForAll result = new DashboardItems.ForAll();
            try
            {
                using (IDbConnection DBContext = new SqlConnection(ClsCommon.ConnectionString()))
                {
                    ...
                    DBContext.Open();
                    using (var reader = DBContext.QueryMultiple("spu_GetDashboard_Common", param: param, commandType: CommandType.StoredProcedure, commandTimeout: commandTimeout))
                    {
                        result = reader.Read<DashboardItems.ForAll>().FirstOrDefault();
                    }
                    DBContext.Close();
                }
            }
            ...
            {
                Common_SPU.LogError(ex.Message.ToString(), ex.ToString(), "GetDashboard_NSM", "spu_GetDashboardItems", "DataModal", Modal.LoginID, Modal.IPAddress);
            }
            return result;
        }
