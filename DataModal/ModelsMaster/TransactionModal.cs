...
namespace DataModal.ModelsMaster
{
    public class TransactionModal : ITransactionHelper
    {
        ...
        public List<TourPlan.List> GetTourPlanList(Tab.Approval Modal)
        {

            List<TourPlan.List> result = new List<TourPlan.List>();
            try
            {
                using (IDbConnection DBContext = new SqlConnection(ConnectionStrings))
                {
                    ...
                    DBContext.Open();
                    using (var reader = DBContext.QueryMultiple("spu_GetTourPlanList", param: param, commandType: CommandType.StoredProcedure, commandTimeout: commandTimeout))
                    {
                        result = reader.Read<TourPlan.List>().ToList();
                    }
            ...
            {
                Common_SPU.LogError(ex.Message.ToString(), ex.ToString(), "spu_GetTourPlanList", "GetTourPlanList", "DataModal", Modal.LoginID, Modal.IPAddress);
            }
            return result;
        }

        public TourPlan.Add GetTourPlan(GetResponse Modal)
        {

            TourPlan.Add result = new TourPlan.Add();
            try
            {
                using (IDbConnection DBContext = new SqlConnection(ConnectionStrings))
                {
                    int commandTimeout = 0;
                    var param = new DynamicParameters();
                    param.Add("@TourPlanID", dbType: DbType.Int64, value: Modal.ID, direction: ParameterDirection.Input);
                    param.Add("@LoginID", dbType: DbType.Int64, value: Modal.LoginID, direction: ParameterDirection.Input);

                    DBContext.Open();
                    using (var reader = DBContext.QueryMultiple("spu_GetTourPlan", param: param, commandType: CommandType.StoredProcedure, commandTimeout: commandTimeout))
                    {
                        result = reader.Read<TourPlan.Add>().FirstOrDefault();
                        if (result == null)
                        {
                            result = new TourPlan.Add();
                            result.TourPlanID = 0;
                        }
            ...
            {
                Common_SPU.LogError(ex.Message.ToString(), ex.ToString(), "spu_GetTourPlan", "GetTourPlan", "DataModal", Modal.LoginID, Modal.IPAddress);
            }
            return result;
        }

        public PostResponse fnSetTourPlan(TourPlan.Add modal)
        {
            ...
            using (SqlConnection con = new SqlConnection(ConnectionStrings))
            {
                ...
                {
                    con.Open();
                    using (SqlCommand command = new SqlCommand("spu_SetTourPlan", con))
                    {
                        SqlDataAdapter da = new SqlDataAdapter();
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.Add("@TourPlanID", SqlDbType.Int).Value = modal.TourPlanID ?? 0;
                        ...
                        command.Parameters.Add("@AreaID", SqlDbType.Int).Value = modal.AreaID ?? 0;
        ...
        public List<TourPlan_Import.List> GetTourPlanImportList(GetResponse Modal)
        {
            ...
            try
            {
                using (IDbConnection DBContext = new SqlConnection(ConnectionStrings))
                {
                    ...
                    var param = new DynamicParameters();
                    DBContext.Open();
                    using (var reader = DBContext.QueryMultiple("spu_GetTourPlanImportList", commandType: CommandType.StoredProcedure, commandTimeout: commandTimeout))
                    {
                        result = reader.Read<TourPlan_Import.List>().ToList();
                    }
        ...
        public PostResponse SetTourPlan_Import(DataSet TempDataset, GetResponse getResponse)
        {
            PostResponse Result = new PostResponse();
            Result.SuccessMessage = "Error occurred while saving into EMP import";
            using (SqlConnection con = new SqlConnection(ConnectionStrings))
            {
                ...
                {
                    con.Open();
                    using (SqlCommand command = new SqlCommand("spu_SetTourPlanImport", con))
                    {
                        SqlDataAdapter da = new SqlDataAdapter();
                        TempDataset.Tables[0].Rows.RemoveAt(0);
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.Add("@LoginID", SqlDbType.Int).Value = getResponse.LoginID;
                        command.Parameters.Add("@TourPlanImportType", SqlDbType.Structured).Value = TempDataset.Tables[0];
                        command.CommandTimeout = 0;
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
        ...
        public PostResponse UploadTourPlanImportExcelFile(HttpPostedFileBase file, string SheetName, GetResponse getResponse)
        {
            PostResponse Result = new PostResponse();
            DataSet TempDataset = default(DataSet);
            string ExcelFileName = null;
            ...
            try
            {
                ExcelFileName = "TourPlanImportData";
                DirectotyPath = ConfigurationManager.AppSettings["ApplicationPath_Physical"] + "\\Attachments\\ImportExcels";

                if (!System.IO.Directory.Exists(DirectotyPath + "\\TourPlanImportData"))
                {
                    System.IO.Directory.CreateDirectory(DirectotyPath + "\\TourPlanImportData");
                }
                FileExtension = System.IO.Path.GetExtension(file.FileName);
                if (FileExtension == ".xlsx" || FileExtension == ".xls")
                {

                    file.SaveAs(DirectotyPath + "\\TourPlanImportData\\" + ExcelFileName + "" + FileExtension);
                    TempDataset = clsDataBaseHelper.GetExcelDataAsDataSet(DirectotyPath + "\\TourPlanImportData\\" + ExcelFileName + "" + FileExtension, SheetName.Replace("'", "''").Replace("$", "") + "$");
                    ...
                    if ((TempDataset != null))
                    {
                        ...
                        if (TempDataset.Tables[0].Columns.Count > 0)
                        {
                            Result = SetTourPlan_Import(TempDataset, getResponse);
                        }
                    }
                }
            }
        ...
        public PostResponse ClearTourPlanImportTemp(GetResponse getResponse)
        {
            PostResponse Result = new PostResponse();
            try
            {
                string SQL = "Truncate table TouPlan_Import";
                clsDataBaseHelper.ExecuteNonQuery(SQL);
                Result.Status = true;
                Result.SuccessMessage = "data cleared";
            }
        ...
        public PostResponse SetTourPlanFromTouPlanImport(GetResponse getResponse)
        {
            PostResponse Result = new PostResponse();
            using (SqlConnection con = new SqlConnection(ConnectionStrings))
            {
                ...
                {
                    con.Open();
                    using (SqlCommand command = new SqlCommand("spu_SetTourPlanFromTourPlanImport", con))
                    {
                        SqlDataAdapter da = new SqlDataAdapter();
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.Add("@createdby", SqlDbType.Int).Value = getResponse.LoginID;
        ...
        public PJPEntry.Add GetPJPEntryAdd(GetResponse Modal, long PJPPlanID, long PJPEntryID, long SSRTourPlanID)
        {
            ...
            try
            {
                using (IDbConnection DBContext = new SqlConnection(ConnectionStrings))
                {
                    ...
                    param.Add("@SSRTourPlanID", dbType: DbType.Int64, value: SSRTourPlanID, direction: ParameterDirection.Input);
                    param.Add("@LoginID", dbType: DbType.Int64, value: Modal.LoginID, direction: ParameterDirection.Input);
                    ...
                    using (var reader = DBContext.QueryMultiple("spu_GetPJPEntry", param: param, commandType: CommandType.StoredProcedure, commandTimeout: commandTimeout))
                    {
                        ...
                        if (result == null)
                        {
                            ...
                            result.SSRTourPlanID = SSRTourPlanID;
                        }
                        if (!reader.IsConsumed)
                        {
            ...
            using (SqlConnection con = new SqlConnection(ConnectionStrings))
            {
                try
                {
                    ...
                    using (SqlCommand command = new SqlCommand("spu_SetPJPEntry", con))
                    {
                        ...
                        command.Parameters.Add("@SSRTourPlanID", SqlDbType.Int).Value = model.SSRTourPlanID ?? 0;
